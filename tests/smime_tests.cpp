...
BOOST_AUTO_TEST_CASE(smime)
{
    ...
    BOOST_REQUIRE_NO_THROW(encrypted_smime = plexus::utils::smime_encrypt(message, "./certs/server.crt"));
    BOOST_REQUIRE_NO_THROW(signed_smime = plexus::utils::smime_sign(encrypted_smime, "./certs/client.crt", "./certs/client.key"));

    BOOST_REQUIRE_NO_THROW(verified_smime = plexus::utils::smime_verify(signed_smime, "./certs/client.crt", "./certs/ca.crt"));
    BOOST_REQUIRE_NO_THROW(decrypted_smime = plexus::utils::smime_decrypt(verified_smime, "./certs/server.crt", "./certs/server.key"));
    
    BOOST_REQUIRE_EQUAL(decrypted_smime, message);
